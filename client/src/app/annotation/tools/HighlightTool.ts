...
import {Canvas as ICanvas, Rect as IRect, Object as IObject} from "fabric";
...
export class HighlightTool implements Tool {
    ...
    static createAnnotation(rects: IRect[], fill: string): HighlightAnnotation {

        let rs: any[] = [];

        for (let r of rects) {
            rs.push({
                x: r.getLeft() * am.getScaleValue(),
                y: r.getTop() * am.getScaleValue(),
                w: r.getWidth() * am.getScaleValue(),
                h: r.getHeight() * am.getScaleValue()
            });
        }
